...
void CardinalityEstimator::initNodeIDDom(
    const QueryGraph& queryGraph, transaction::Transaction* transaction) {
...
double CardinalityEstimator::getExtensionRate(const RelExpression& rel,
    const NodeExpression& boundNode, transaction::Transaction* transaction) {
