...
bool ListColumnChunk::isOffsetsConsecutiveAndSortedAscending(
    ...
    for (auto i = 0u; i < numValues; i++) {
        if (nullChunk->isNull(i)) {
            ...
            dataColumnChunk->append(
                listDataColumnChunk->dataColumnChunk.get(), startOffset, listSize);
